...
```{r question_1}
question_checkbox("What are the possible methods for creating a background area available in `create_background()`?",
...
This means to model somewhat more complicated functions of the environment we need to create polynomial terms from our predictors, which can be done in the recipe using `step_poly()`. We will also want to do our usual transformation and normalization. Here we use the `all_predictors()` selection helper. This just tells the recipe to apply the step to all predictor variables. We could have also used `BIO1:BIO19` to accomplish the same thing.
