...
description: SDM Course Week 2 Assignment
---
...
Please complete the assignment by making sure you follow the instructions in each of the Exercises (there are three of them, and should be sections titled 'Exercise 1', Exercise 2', and 'Exercise 3'). Make sure you modify the code in any code boxes that you are asked to modify. If the code remains unchanged you will not get marks for that exercise. Be careful with the 'Start Over' button as this will replace any code you wrote with what was originally in the code box.
...
Using the code box below, change the overall look of the plot by adding a theme. Use any theme you want. You can try out a bunch and find the one you like the best if you want.

```{r plot1_1, exercise=TRUE}
ggplot(penguins, aes(x = bill_length_mm, y = body_mass_g)) +
  geom_point()
```
...
<div id="plot1_1-hint">
**Hint:** Try typing `theme_` and the autocomplete should offer suggestions.
</div>
...
See the [Week 2 Lecture Slides](https://docs.google.com/presentation/d/1yuUylI9WMYkMfbrUXFOsR9G3vpfLe5QOLakulTOsNGc/edit?usp=sharing) for definitions of this terminology.
...
That actually won't work! That is because `esquisse` is what's called a shiny app, and this tutorial is already a shiny app. A shiny app can't run a shiny app. But we can run our code in 'RStudio'. Paste the code above into the console to try it out! 

### Exercise 2

Either use `esquisse` or simply modify the code below to create a plot using three or four variables. Don't use the exact same variables as already there. Change at least one of the variables to a new one, and change at least one of the aesthetic mappings (e.g. map sex to facte instead of colour, etc.)
...
<div id="plot5-hint">
**Hint:** You can use the 'environment browser' in RStudio to look at the `penguins` dataset and see what other variables are available for use in the plot.
</div>
...
See [Week 2 Lecture Slides](https://docs.google.com/presentation/d/1yuUylI9WMYkMfbrUXFOsR9G3vpfLe5QOLakulTOsNGc/edit?usp=sharing) for more info.

The complete code used to get from `penguins_raw` to `penguins` is in the code box below. You don't need to know or understand all of these steps but it should give you and idea of the power of `tidyverse` verbs and the pipe for concisely wrangling data. Try running the code an comparing `penguins_df` to `penguins_raw`. They should be the same!
...
In the code box below, modify the code to summarise over a different set of grouping variables (change at least one of species or sex). Also, summarize a different variable or set of variables (change `body_mass_g` to something else). There is a bonus mark if you can correctly use a different summary function besides `mean()` or `var()`.

```{r summary_penguin2, exercise=TRUE}
...
```{r summary_penguin2-check}
ls()
```

That's it for today! Please go to the 'Submit' section and generate your hash code.
